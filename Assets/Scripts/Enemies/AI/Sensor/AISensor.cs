...
public class AISensor : MonoBehaviour
{
    ...
    public List<AttackStack> attacks;
    private int count;
    Collider[] playerColliders = new Collider[50];
    Collider[] attackColliders = new Collider[50];

    private bool PlayerInSight(Transform player)
    {
    ...
    public List<GameObject> ScanForPlayer()
    {
        count = Physics.OverlapSphereNonAlloc(transform.position, detectionRadius, playerColliders, LayerMask.GetMask("Player"), QueryTriggerInteraction.Collide);

        targets.Clear();
        for (int i = 0; i < count; ++i)
        {
            GameObject obj = playerColliders[i].gameObject;

            if (obj.CompareTag("Player") && PlayerInSight(obj.transform))
            {
                targets.Add(obj);
            }
        }
    ...
    public List<AttackStack> ScanForAttack()
    {
        count = Physics.OverlapSphereNonAlloc(transform.position, detectionRadius, attackColliders, LayerMask.GetMask("Attack"), QueryTriggerInteraction.Collide);

        attacks.Clear();
        for (int i = 0; i < count; ++i)
        {
            AttackStack obj = attackColliders[i].GetComponent<AttackStack>();

            if (obj.CompareTag("Attack") && PlayerInSight(obj.transform))
            {
                attacks.Add(obj);
            }
        }
