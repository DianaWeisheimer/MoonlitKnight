...
public class CompanionRoleKnight : CompanionRoleProfile
{
    ...
    public override void ExecuteEngage(CompanionBrain brain)
    {
        ...
        {
            if(Random.Range(0,100) < 25) Attack(brain, true);
            else Attack(brain, false);
            ScheduleAttack();
        }   
    }
    ...
    public void Attack(CompanionBrain brain, bool heavyAttack)
    {
        if (heavyAttack)
        {
            ...
            brain.character.equipment.CalculateWeaponDamage(-65);
        }

        else
        {
            brain.character.stats.CheckStaminaCost(25);
            brain.character.equipment.CalculateWeaponDamage(-85);
        }
